...
const std::string Users::Cols::_hashed_password = "\"hashed_password\"";
...
Users::Users(const Json::Value &pJson, const std::vector<std::string> &pMasqueradingVector) noexcept(false)
{
    ...
    if(!pMasqueradingVector[3].empty() && pJson.isMember(pMasqueradingVector[3]))
    {
        ...
        {
            hashedPassword_=std::make_shared<std::string>(pJson[pMasqueradingVector[3]].asString());
        }
    }
...
const std::string &Users::getValueOfHashedPassword() const noexcept
{
    static const std::string defaultValue = std::string();
    if(hashedPassword_)
...
const std::shared_ptr<std::string> &Users::getHashedPassword() const noexcept
{
    return hashedPassword_;
}
void Users::setHashedPassword(const std::string &pHashedPassword) noexcept
{
    hashedPassword_ = std::make_shared<std::string>(pHashedPassword);
    dirtyFlag_[3] = true;
}
void Users::setHashedPassword(std::string &&pHashedPassword) noexcept
{
    ...
    const std::vector<std::string> &pMasqueradingVector) const
{
    ...
    if(pMasqueradingVector.size() == 5)
    {
        ...
        if(!pMasqueradingVector[3].empty())
        {
            if(getHashedPassword())
            {
                ret[pMasqueradingVector[3]]=getValueOfHashedPassword();
            }
            else
            {
...
bool Users::validateJsonForCreation(const Json::Value &pJson, std::string &err)
{
    ...
    if(pJson.isMember("hashed_password"))
    {
        if(!validJsonOfField(3, "hashed_password", pJson["hashed_password"], err, true))
            return false;
    }
    else
    {
...
bool Users::validateJsonForUpdate(const Json::Value &pJson, std::string &err)
{
    ...
    if(pJson.isMember("hashed_password"))
    {
        if(!validJsonOfField(3, "hashed_password", pJson["hashed_password"], err, false))
            return false;
    }
    if(pJson.isMember("created_at"))
    {
